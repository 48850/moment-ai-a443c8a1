...
async function ttsWithRetry(text: string, voiceId: string, prev?: string, next?: string, attempt = 0): Promise<string> {
  try {
    return await tts(text, voiceId, prev, next);
  } catch (e) {
    const msg = e instanceof Error ? e.message : "";
    if (attempt < 4 && /429|5\d\d/.test(msg)) {
      const backoff = 600 * Math.pow(2, attempt) + Math.random() * 400;
      await new Promise((r) => setTimeout(r, backoff));
      return ttsWithRetry(text, voiceId, prev, next, attempt + 1);
    }
    throw e;
  }
}
...
async function attachVoiceover(segments: any[]): Promise<{ segments: any[]; ok: boolean; error?: string }> {
  // ElevenLabs caps concurrency at 4 on lower tiers — run sequentially to be safe.
  const results: any[] = [];
  try {
    for (let i = 0; i < segments.length; i++) {
      ...
      const audio_base64 = await ttsWithRetry(s.line, voiceId, prev, next);
      results.push({ ...s, audio_base64 });
    }
    ...
    // Return any segments we managed to voice, fall back to text for the rest
    const merged = segments.map((s, i) => results[i] ?? s);
    return { segments: merged, ok: results.length > 0, error: msg };
  }
}
